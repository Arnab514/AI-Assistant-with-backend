...
const App = () => {
  ...
  const handleUserInput = async e => {
    ...
    try {
      if (conversationState.stage === "initial") {
        const rankedOptions = await generateOptions(currentQuestion)

        setChatHistory(prev => [
          ...prev,
          {
            type: "options",
            content: rankedOptions,
            id: Date.now() + 1
          }
        ])
        ...
        setConversationState({
          stage: "awaiting_selection",
          currentOptions: rankedOptions,
          selectedOptions: []
        })
      } else if (conversationState.stage === "awaiting_selection") {
        if (currentQuestion.toLowerCase().includes("explain")) {
          const optionNumber = parseInt(
            currentQuestion.match(/\d+/)?.[0] || "0"
          )
          if (
            optionNumber > 0 &&
            optionNumber <= conversationState.currentOptions.length
          ) {
            const option = conversationState.currentOptions[optionNumber - 1]
            setChatHistory(prev => [
              ...prev,
              {
                type: "answer",
          ...
          if (selectedNumbers) {
            const selected = selectedNumbers.map(
              num => conversationState.currentOptions[num - 1].option
            )
            ...
            setChatHistory(prev => [
              ...prev,
              {
                type: "answer",
                content:
                  "Please select exactly 2 different numbers between 1 and 3 (e.g., '1 and 2')",
                id: Date.now() + 1
              }
            ])
          }
  ...
  const handleExplanationRequest = option => {
    setChatHistory(prev => [
      ...prev,
      {
        type: "answer",
    ...
    if (chat.type === "options") {
      return (
        <div className="space-y-4">
          {chat.content.map((option, index) => (
            <RankedOptionCard
              key={index}
              option={option.option}
              score={option.score}
              rank={index + 1}
              onExplanationRequest={() => handleExplanationRequest(option)}
            />
          ))}
          <p className="text-gray-300 mt-4">
            Please select 2 options by typing their numbers (e.g., '1 and 2' or
            '2 and 3')
          </p>
        </div>
      )
    ...
    <div className="fixed inset-0 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-gray-100 animate-gradient">
      <div className="h-full max-w-4xl mx-auto flex flex-col p-3">
          ...
          {showWelcome && chatHistory.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-6">
              <div className="glass-morphism bg-gray-800/80 rounded-xl p-8 max-w-2xl border border-gray-700 shadow-2xl transform hover:scale-105 transition-all duration-300">
                ...
                <p className="text-gray-300 mb-6">
                  I'm here to help! Ask me anything and I'll provide interactive
        ...
        <form
          ...
          className="glass-morphism bg-gray-800/50 rounded-xl shadow-xl p-4 border border-gray-700"
        >
          <div className="flex gap-2">
            <textarea
              required
